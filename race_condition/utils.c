...
int semaphore_new(key_t key) 
{
	...
	if ((s = semget(key, 1, IPC_CREAT | 0666 )) == FAILURE) {
		fprintf(stderr, "The semget() function "
		                "has failed: %s!\n", strerror(errno));
		exit(EXIT_FAILURE);
	}
	return s;
...
void semaphore_destroy(int semaphore) 
{
		...
		fprintf(stderr, "The semctl() function "
		                "has failed: %s!\n", strerror(errno));
                exit(EXIT_FAILURE);
        }
}
...
void p(int semaphore, unsigned short subtract) 
{
	...
	if (semop(semaphore, &sem_lock, 1) == FAILURE) {
		fprintf(stderr, "The semop() function "
		                "has failed: %s!\n", strerror(errno));
		exit(EXIT_FAILURE);
	}
}
...
int create_shared_memory(key_t key)
{
	...
	if ((shm = shmget(key, sizeof(int), IPC_CREAT | 0666)) == FAILURE) {
		fprintf(stderr, "The shmget() function "
		                "has failed: %s!\n", strerror(errno));
		exit(EXIT_FAILURE);
	}
...
int *associate_shared_memory(int shm)
{
	...
	if ((shm_addr = (int *) shmat(shm, NULL, 0)) == (int *) FAILURE) {
		fprintf(stderr, "The shmat() function "
		                "has failed: %s!\n", strerror(errno));
		exit(EXIT_FAILURE);
	}
...
void shared_memory_destroy(int shm)
{
	if (shmctl(shm, IPC_RMID, NULL) != 0) {
		fprintf(stderr, "The shmctl() function "
		                "has failed: %s!\n", strerror(errno));
		exit(EXIT_FAILURE);
	}
}
